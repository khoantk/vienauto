...
namespace Vienauto.Core.Nhibernate.CachingConfiguration
{
    public interface INHibernateConfigurationCache
    {
        ...
        Configuration LoadConfiguration(string configKey, string configPath, string[] mappingAssemblies, Assembly assembly);
        ...
        void SaveConfiguration(string configKey, Configuration config, Assembly assembly);
    }
    ...
    public class NHibernateConfigurationFileCache : INHibernateConfigurationCache
    {
        ...
        public Configuration LoadConfiguration(string configKey, string configPath, string[] mappingAssemblies, Assembly assembly)
        {
            string cachePath = CachedConfigPath(configKey, assembly);
        ...
        /// <param name="configKey">Key value to provide a unique name to the cached <see cref="Configuration"/>.</param>
        /// <param name="config">Configuration object to save.</param>
        public void SaveConfiguration(string configKey, Configuration config, Assembly assembly)
        {
            string cachePath = CachedConfigPath(configKey, assembly);
        ...
        protected virtual string CachedConfigPath(string configKey, Assembly assembly)
        {
            var fileName = string.Format("{0}-{1}.bin", configKey, assembly.CodeBase.GetHashCode());
            var path =
