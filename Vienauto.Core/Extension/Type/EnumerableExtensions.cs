...
namespace Vienauto.Core.Extension
{
    public static class EnumerableExtensions
    {
        ...
        public static IList<T> Filter<T>(this IEnumerable<T> items, Func<T, bool> predicate)
        {
            return items.Where(predicate).ToList();
        }

        public static IList<TDes> ToList<TSource, TDes>(this IEnumerable<TSource> items, Func<TSource, TDes> func)
        {
            var results = new List<TDes>();
            items.ForEach(i => results.Add(func(i)));
            return results;
        }

        public static IList<T> Default<T>(this IList<T> items, T item)
        {
